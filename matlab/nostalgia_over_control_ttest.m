...
function nostalgia_over_control_ttest(~) 
    close all
    cfg = read_config();

    if nargin < 1
        countries = cfg.countries;
        output_folder = [cfg.pic_pq1and3, 'nostovercont'];
        ...
        output_folder = [cfg.pic_pq1and3, 'nostovercont_all'];
        if ~exist(output_folder, 'dir')
            mkdir(output_folder);
        else
            delete(fullfile(output_folder, '*.png'));
    ...
    plot_grid(results, 'diff', M_diff, diff_cmap, base2, mask, ...
        'Nostalgia - Control — Mean difference', ...
